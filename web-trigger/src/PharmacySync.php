...
class PharmacySync {
    ...
    public function __construct() {
        $this->elastic = Elasticsearch\ClientBuilder::create()
        ...
        try {
            ...
            $stmt = $db->prepare("
                SELECT pharmacy_id, pharmacy_name, phone_number, 
                       email, address, ST_X(location) as longitude, 
    ...
    private function getActivePharmacyIds(): array {
        $db = DatabasePool::getConnection();
        try {
            return $db->query("SELECT pharmacy_id FROM pharmacy WHERE is_active = 1")
    ...
    private function getPharmacyCredentials(int $pharmacyId): array {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => ["Authorization: Bearer {$this->dopplerToken}"],
